...
Задача актуальна, так как реазизовывает достаточно [извеcтные] (http://citeseerx.ist.psu.edu/viewdoc/download?doi=10...) алгоритмы в одной из самой популярной библиотеки для ML, которые дают хорошие результаты, а так же дополняют функционал самой библиотеки.
